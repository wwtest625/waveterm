...
全新引入的卡片式终端体验，灵感来自 Warp：

- **对话式展示** — 命令输入（右侧气泡）+ 命令输出（左侧气泡），一目了然
- **流式输出** — 命令执行中实时追加输出，像聊天消息一样流畅
- **智能折叠** — 长输出自动折叠，支持展开、搜索、复制
- **状态可见** — 显示退出码、执行耗时，失败命令显著标记
- **目录延续** — 当前工作目录（cwd）在卡片间自动延续并显示
- **上下文标签** — 显示并可编辑"上下文标签"（如容器名/Pod名），跨重启保持
- **自动降级** — 遇到交互式命令（TUI）时自动切回传统终端视图

点击终端 Header 即可在"卡片终端"和"普通终端"之间切换。

### 🐳 Docker 侧边栏面板

内置 Docker 管理，无需离开终端：

- **容器管理** — 列表显示容器名、镜像、状态、端口，支持 Start/Stop/Restart/Delete
- **镜像管理** — 列表显示镜像名、Tag、大小，支持拉取和删除镜像
- **日志查看** — 点击容器查看实时日志
- **终端进入** — 双击或右键 Exec 进入容器内部

### 🖥️ Tmux 侧边栏管理器

可视化管理 tmux 会话和窗口：

- **Session 管理** — 列出、创建并进入、重命名、Detach、Kill
- **Window 管理** — 选中 Session 后可管理其 Windows
- **一键操作** — 点击按钮即可执行操作，无需记忆 tmux 命令

### 🤖 命令自动补全

智能终端命令补全：

- **Shell 命令补全** — 常用 Shell 命令自动补全
- **Docker 命令补全** — Docker 命令、容器名、镜像名智能补全
- **上下文感知** — 根据当前环境提供相关补全建议
...
- **Codex 集成** - 支持 Codex App-Server 本地 AI Agent 终端
