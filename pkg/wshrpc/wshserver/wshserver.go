...
func (ws *WshServer) UpdateKnownHostKeyCommand(ctx context.Context, data wshrpc.UpdateKnownHostKeyData) error {
	...
	opts, err := remote.ParseOpts(data.Host)
	if err != nil {
		return fmt.Errorf("failed to parse host: %w", err)
	}
	hostPattern := opts.SSHHost

	sshKeywords, err := remote.FindSshConfigKeywords(hostPattern)
	if err != nil {
		sshKeywords, err = remote.FindSshDefaults(hostPattern)
		if err != nil {
